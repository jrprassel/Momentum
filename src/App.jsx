            ...
            <div style={{ marginBottom: 24 }}>
              ...
              {data.customHabits.map(h => (
                <div key={h.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #1a1a1a" }}>
                  <div style={{ fontSize: 13, color: "#888" }}>{h.name}</div>
                  <button onClick={() => removeHabit(h.id)} style={{ background: "none", border: "none", color: "#3a3a3a", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>Remove</button>
                </div>
              ))}
            </div>
          )}
